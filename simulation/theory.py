...
class HeatSolver:
    ...
    def exact_solution(self, x, t):
        """
        Exact solution for u_t = μ u_xx + a u with u(x,0) = sin(πx) and u(0,t) = u(L,t) = 0.
        u(x,t) = exp((a - μ*π²)t) * sin(πx)
        """
        return np.exp((self.a - self.mu * (np.pi**2)) * t) * np.sin(np.pi * x)
    
    def solve_crank_nicolson(self, M, N, return_all=False):
        """
        Solve using Crank-Nicolson scheme with M spatial points and N time steps.
